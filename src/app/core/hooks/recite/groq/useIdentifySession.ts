/**
 * IDENTIFY SESSION — Groq engine's own copy of the "which verse am I
 * reciting" state machine.
 *
 * This file is a fork of ../deepgram/useIdentifySession.ts — they started
 * identical but are independently editable per-engine copies, not a shared
 * module. Do not "deduplicate" them back together: the whole point of the
 * per-engine folder split is that a Deepgram-specific change to identify
 * timing/behavior can never alter Groq's, and vice versa.
 */
...
export interface IdentifySession {
  ...
  /** Feed one segment of recognized text (a Groq chunk's text, or a
   *  Deepgram final's text) into the identify state machine. */
  handleIdentifyChunk: (text: string) => Promise<void>;
...
export function useIdentifySession(deps: IdentifySessionDeps): IdentifySession {
  ...
  const handleIdentifyChunk = useCallback(
    async (text: string) => {
      ...
      // whole-Quran identification to play out.
