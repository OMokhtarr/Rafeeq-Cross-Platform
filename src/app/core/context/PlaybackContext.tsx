 ...
 */
function getQueuePageMarkers(queue: VerseKey[]): Array<{ page: number; aya: number }> {
...
export const PlaybackProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [currentReciterId, setCurrentReciterId] = useState(DEFAULT_OPTS.reciter);
  // Cached page markers for the current queue (recomputed only when queue identity changes)
  const pageMarkersCache = useRef<{ firstAya: number; lastAya: number; len: number; markers: Array<{ page: number; aya: number }> } | null>(null);
  ...
  useEffect(() => {
    ...
    if (q.length > 0) {
      ...
      if (!cached || cached.firstAya !== firstAya || cached.lastAya !== lastAya || cached.len !== len) {
    ...
    const currentPage =
      !isNaN(suraNum) && !isNaN(ayaNum) ? estimatePageForVerse(suraNum, ayaNum) : 0;
  ...
  }, [isNative, currentReciterId, queue.queue, queue.state.isPlaying, queue.state.currentVerse, queue.state.durationMs]);
    ...
    DrivingMode.addListener("carAction", (event) => {
      switch (event.action) {
        ...
        case "selectSurah": {
          ...
          const verseQueue: VerseKey[] = Array.from({ length: count }, (_, i) => ({
            sura: surahNum,
            aya: i + 1,
          }));
          if (event.reciter) {
            queue.setReciter(event.reciter);
            setCurrentReciterId(event.reciter);
          }
          queue.start(verseQueue).catch(() => {});
          break;
        }
        case "jumpToAya": {
          // Page button tapped — find the queue index whose aya matches the marker aya.
          if (event.aya == null) break;
          const targetAya = event.aya;
          const targetIndex = activeQueueRef.current.findIndex((v) => v.aya === targetAya);
          if (targetIndex !== -1) queue.jumpToIndex(targetIndex);
          break;
        }
      }
    }).then((handle) => {
