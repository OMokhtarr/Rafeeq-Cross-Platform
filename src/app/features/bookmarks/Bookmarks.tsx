...
const Bookmarks: React.FC = () => {
  ...
  // ── Navigate to page ──
  const goToPage = (page: number) => history.push(`/viewer?page=${page}`);
        ...
        <div className="bm-page" dir={isRTL ? "rtl" : "ltr"}>
          ...
          <header className="bm-header">
            <button
              className="bm-back"
              onClick={() =>
                history.length > 1 ? history.goBack() : history.replace("/viewer")
              }
            ...
            <div className="bm-back" aria-hidden style={{ visibility: "hidden" }} />
          </header>

          <div className="bm-body">
            {/* ══ Section 1: Saved Verses ══ */}
            <section className="bm-section">
              <div className="bm-section-header">
                ...
                <span className="bm-section-count">{bookmarkedKeys.length}</span>
              </div>
              ...
              {bookmarkedKeys.length === 0 ? (
                ...
                <ul className="bm-list">
                  {bookmarkedKeys.map((vk) => {
                    const { sura, aya, surahName, page } = verseKeyToPageAndName(vk, lang);
                    return (
                      <li key={vk} className="bm-row">
                        ...
                        >
                          ...
                          <div className="bm-row-info">
                            <span className="bm-row-primary" lang={lang === "ar" ? "ar" : undefined}>
                              {surahName}
                            </span>
                            <span className="bm-row-secondary">
                              {lang === "ar"
                                ? `آية ${toHindiNumbers(aya)} • صفحة ${toHindiNumbers(page)}`
                                : `Verse ${aya} • Page ${page}`}
                            </span>
                          </div>
                          <svg
            ...
            {/* ══ Section 2: Recitation Sessions ══ */}
            <section className="bm-section">
              <div className="bm-section-header">
                <span className="bm-section-icon">🎙️</span>
                <h2 className="bm-section-title">
                  {lang === "ar" ? "آخر جلسات التلاوة" : "Recent Recitation Sessions"}
                </h2>
              ...
              {sessions.length === 0 ? (
                ...
                <ul className="bm-list">
                  {sessions.map((session) => {
                    const { sura, aya, surahName, page } = verseKeyToPageAndName(
                      session.verseKey,
                      lang,
                    );
                    return (
                      <li key={session.id} className="bm-row">
                        ...
                        >
                          ...
                          <div className="bm-row-info">
                            ...
                            <span className="bm-row-secondary">
                              {lang === "ar"
                                ? `آية ${toHindiNumbers(aya)} • ${formatElapsed(session.elapsedSeconds, lang)}`
                                : `Verse ${aya} • ${formatElapsed(session.elapsedSeconds, lang)}`}
                            </span>
