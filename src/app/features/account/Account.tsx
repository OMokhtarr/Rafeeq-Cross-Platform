...
import { fetchStreaks, type Streak, UserApiError } from "../../core/services/api/user-api.client";
import "./Account.css";
...
const Account: React.FC = () => {
      ...
      .catch((err) => {
        if (!cancelled) {
          if (err instanceof UserApiError && (err.status === 401 || err.status === 403)) {
            setError(lang === "ar" ? "يرجى تسجيل الدخول لعرض بياناتك" : "Please sign in to view your data");
          } else {
            setError(lang === "ar" ? "تعذر تحميل البيانات" : "Could not load data");
          }
          setLoading(false);
        }
      });
  ...
  const longestStreak = streaks.reduce((max, s) => (s.days > max ? s.days : max), 0);
        ...
        <div className="account-page" dir={isRTL ? "rtl" : "ltr"}>
          ...
          <header className="account-header">
            ...
            <div className="account-header-titles">
              <h1 className="account-title">{lang === "ar" ? "حسابي" : "My Account"}</h1>
              <p className="account-subtitle">{lang === "ar" ? "الإحصاءات والإنجازات" : "Stats & Achievements"}</p>
            </div>
          ...
          <div className="account-body">
            ...
            <div className="account-avatar-section">
              ...
              <p className="account-guest-label">{lang === "ar" ? "زائر" : "Guest"}</p>
                ...
                <>
                  ...
                  <div className="streak-stats-row">
                    <div className="streak-stat">
                      ...
                      <div className="streak-stat-label">{lang === "ar" ? "أطول سلسلة" : "Longest"}</div>
                    </div>
                    <div className="streak-stat-divider" />
                    <div className="streak-stat">
                      <div className="streak-stat-value">{streaks.length}</div>
                      <div className="streak-stat-label">{lang === "ar" ? "سلاسل سابقة" : "Total streaks"}</div>
                    </div>
                    <div className="streak-stat-divider" />
                    <div className="streak-stat">
                      ...
                      <div className="streak-stat-label">{lang === "ar" ? "حالة اليوم" : "Today"}</div>
                    </div>
                  </div>
                  ...
                  {streaks.length > 0 ? (
                        ...
                        <li key={s.id} className="streak-row">
                          ...
                          <div className="streak-row-info">
                            ...
                            <span className="streak-row-type">
                              {s.type === "QURAN"
                                ? (lang === "ar" ? "قراءة القرآن" : "Quran reading")
                                : s.type}
                            </span>
                          </div>
                          ...
                          <span className={`streak-status-badge ${s.status === "ACTIVE" ? "streak-status-active" : "streak-status-broken"}`}>
                            {s.status === "ACTIVE"
                              ? (lang === "ar" ? "نشط" : "Active")
                              : (lang === "ar" ? "منقطع" : "Broken")}
                          </span>
                        </li>
                      ))}
                    </ul>
                    ...
                    <div className="streak-empty">
                      <span>🌙</span>
                      <p>{lang === "ar" ? "لا توجد سلاسل قراءة بعد — ابدأ قراءتك اليوم!" : "No reading streaks yet — start reading today!"}</p>
                    </div>
                  )}
                </>
              )}
